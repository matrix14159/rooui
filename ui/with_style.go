...
func (p *WithStyle) SetStyleIf(v *Ref[bool], items ...*Ref[StyleItem]) {
	handler := func(oldVal, newVal any) {
		...
		if newVal.(bool) {
			...
			p.setStyleProperty(p.target, items...)
		} else {
			p.unbindUpdateHandler(p.target, items...)
			p.removeStyleProperty(p.target, items...)
		}
	}
...
func (p *WithStyle) setStyleProperty(target UI, items ...*Ref[StyleItem]) {
	w := dom.GetWindow()
	d := w.Document()
	el := d.GetElementByID(target.GetUIElementId()).(dom.HTMLElement)
	for _, one := range items {
		el.Style().SetProperty(one.Value().Name(), one.Value().ValueWithUnit(), "")
	}
}

func (p *WithStyle) removeStyleProperty(target UI, items ...*Ref[StyleItem]) {
	w := dom.GetWindow()
	d := w.Document()
	el := d.GetElementByID(target.GetUIElementId()).(dom.HTMLElement)
	for _, one := range items {
...
func (p *WithStyle) clear(target UI) {
	...
	p.removeStyleProperty(target, p.StyleItems...)
	for _, group := range p.ConditionStyles {
		p.unbindUpdateHandler(target, group.StyleItems...)
		p.removeStyleProperty(target, group.StyleItems...)
	}
	p.StyleItems = nil
}
